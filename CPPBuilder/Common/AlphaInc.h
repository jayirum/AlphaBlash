#ifndef __ALPHA_INC_H__
#define __ALPHA_INC_H__
...
//	};
	...
	enum EN_ORD_ACTION {  ACTION_NONE, ACTION_NEW, ACTION_CHANGE, ACTION_CLOSE };
	...
	enum EN_PRC_PT_TP	{ TP_PRICE=1, TP_POINT };	// #define FDN_PRC_PT_TP      528
...
#define TIMEFRAME_MN1   "MN1"
	...
	#define WM_EA_LOGONOFF		WM_USER + 425
	#define WM_CHART_DRAW		WM_USER + 426
	#define WM_DELETE_ALL		WM_USER + 427
	#define WM_REG				WM_USER + 428
	#define WM_UNREG			WM_USER + 429
...
public:
	...
	CBlock& operator=(CBlock& rhs)
	{
		m_size 	= rhs.size();
		m_p 	= new char[m_size];
		init();
		memcpy(m_p, rhs.get(), m_size);
	}
	void	init()	{ ZeroMemory(m_p,m_size); }
    char*	p()		{ return m_p;}
	char* 	get()	{ return m_p;}
...
unsigned int	ReportException(DWORD dExitCode, const char* psPos, _Out_ char* pzMsgBuff);
