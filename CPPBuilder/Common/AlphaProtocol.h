...
public:
	...
	void SetVal(int nFd, const char* val){ SetVal(nFd, (char*)val); }
...
typedef int  FIELD_CODE;
typedef map<FIELD_CODE, AnsiString>	MAP_VALUE;
typedef map<FIELD_CODE, AnsiString>::iterator  IT_MAP_VALUE;
class CProtoGetArray
{
public:
	CProtoGetArray();
	~CProtoGetArray();

	void  	Parsing(_In_ AnsiString& data );
	bool    Is_Empty() 	{ return (deq.size()==0); }
	int  	ArraySize() { return deq.size(); }

	bool	GetVal(int idx, int nFd, _Out_ char* pzVal);
	bool	GetVal(int idx, int nFd, _Out_ AnsiString* psVal);
	bool	GetVal(int idx, int nFd, _Out_ int* pnVal);
	bool	GetVal(int idx, int nFd, _Out_ double* pdVal);
private:
	void  Clear();
private:
	deque< MAP_VALUE > deq;
...
//	char*	GetRecvData() { return m_zBuf; }
	...
	bool	GetVal(int nFd, _Out_ AnsiString* psVal);
	bool	GetVal(int nFd, _Out_ long* pnVal);
...
private:
	int		Parsing(_In_  char* pRecvData);
...
//	bool GetUserId(char* pzRecvData, _Out_ string& sUserId);
	...
	bool  GetCode(_In_ char* pzRecvData, _Out_ char* pzPacketCode);
	bool  GetCommandCode(_In_ char* pzRecvData, _Out_ char* pzCode);
...
public:
	...
	bool	GetOnePacketLock(_Out_ int* pnTotalLen, _Out_ char* pOutBuf);
	bool 	GetOnePacket(_Out_ int* pnTotalLen, _Out_ char* pOutBuf);
