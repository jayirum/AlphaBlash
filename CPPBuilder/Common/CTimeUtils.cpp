...
String CTimeUtils::Today_yyyymmdd()
{
	TDateTime dtNow(Now());
	String sRet = dtNow.FormatString("yyyymmdd");
	return sRet;
}
...
String CTimeUtils::Time_hhmmssmmm()
{
	TDateTime dtNow(Now());
	String sRet = dtNow.FormatString("hh:nn:ss:zzz");
	return sRet;
}


String CTimeUtils::Time_hhmmss()
{
	TDateTime dtNow(Now());
	String sRet = dtNow.FormatString("hh:nn:ss");
	return sRet;
}


String CTimeUtils::DateTime_yyyymmdd_hhmmssmmm()
{
   	TDateTime dtNow(Now());
	String sRet = dtNow.FormatString("yyyymmdd_hhnnsszzz");
	return sRet;
}


String CTimeUtils::DateTime_yyyymmdd_hhmmss()
{
   	TDateTime dtNow(Now());
	String sRet = dtNow.FormatString("yyyymmdd_hhnnss");
	return sRet;
}



String CTimeUtils::DateTime_yyyy_mm_dd__hh_mm_ss()
{
   	TDateTime dtNow(Now());
	String sRet = dtNow.FormatString("yyyy:mm:dd hh:nn:ss");
	return sRet;
}
...
//	if(bFull){
//		wsprintf(time, "%04d.%02d.%02d %02d:%02d:%02d.%03d",
...
//}
//
//bool CUtil::Is_Weekend()
//{
//	struct tm today;
//	time_t now = time(0);
//	localtime_s(&today, &now);
//	bool ret = false;
//	if (today.tm_wday == EN_SUNDAY || today.tm_wday == EN_SATURDAY)
//		ret = true;
//	return ret;
//}
//
...
//}
//
//void CUtil::GetStringTrimmed(_In_ char* pOrg, int nLen, _Out_ std::string* pString)
//{
//	char z[128];
//	sprintf(z, "%.*s", nLen, pOrg);
//	CUtil::RTrim(z, strlen(z));
//
//	*pString = std::string(z);
//}
//
...
//
//
//string	CUtil::stringFormat(const char* p, ...)
//{
//	smartptr buf(1024);
//
//	va_list argptr;
...
//	CUtil::TrimAll(szDir, strlen(szDir));
//	if (szDir[0] == '.') {
//		bSameDir = TRUE;
//	}
//	if(strlen(szDir) == 0)
...
//}
//
//VOID CUtil::SplitDataEx(_In_ const char* psData, _In_ const char cDelimeter, _In_ int nSize, _Out_ std::list<std::string>* pListResult)
//{
//	char pData[1024] = { 0, };
...
//}
//
//VOID CUtil::SplitData(_In_ const char* psData, _In_ const char cDelimeter, _Out_ std::list<std::string>* pListResult)
//{
//	char* pFind;
//	char temp[1024];
//	const char* pData = psData;
//
//
//
//	while (TRUE)
//	{
//		pFind = strchr ((char*)pData, cDelimeter);
//
//		// 123/456
//		if (!pFind)
//		{
...
//
//
//VOID CUtil::SplitData(_In_ const char* psData, _In_ const char cDelimeter, _Out_ std::vector<std::string>* pListResult)
//{
//	const char* pFind;
//	char temp[1024];
//	const char* pData = psData;
...
//}
//
//string* CSplit::Split(const string& sOrgData, char cDeli)
//{
//	vector<string> vec;
//	Split(sOrgData, cDeli, vec);
//	m_p = new string[m_nCnt];
...
//}
//
//int CSplit::Split(const string& sOrgData, char cDeli, _Out_ vector<string>& vec)
//{
//	// count the numbers oof the delimieter
//	Clear();
//
//	string sData = sOrgData;
//	if (sData[sData.size()-1] != cDeli)
//		sData.push_back(cDeli);
//
//	size_t prev = 0, found = 0;
//	while (TRUE)
//	{
//		found = sData.find(cDeli, prev);
//		if (found == string::npos)
//			break;
//
//		vec.push_back(sOrgData.substr(prev, found - prev));
//
//		prev = found + 1;
//	}
//
