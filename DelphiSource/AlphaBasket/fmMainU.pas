...
uses
  ...
  ,uBasketCommon,MTLoggerU, uQueueEx, IdGlobal, uConfig, AdvUtil,
...
type
  ...
  TfmMain = class(TForm)
    ...
    procedure btnStartTradeClick(Sender: TObject);
  ...
  private
    ...
    m_csClientCtx     : TRTLCriticalSection;
  ...
  public
    procedure AddMsg(bSucc:boolean;sMsg:string; bShow:boolean=true);
...
uses fmExeUpdateU, fmMT4UpdateU, uPacketProcess, uCtrls
      ,ProtoGetU, uAlphaProtocol, uLoadSymbols
    ;
...
begin
  ...
  s :=  __Get_CFGFile('BROKER_COUNT', 'COUNT', '', False, m_sConfigFile);
  __dataHandler.Set_RealBrokerCnt(strtoint(s));
...
procedure TfmMain.btnStartTradeClick(Sender: TObject);
begin
...
begin
  ...
  if __DataHandler<>NIL  then
    FreeAndNil(__DataHandler);
...
begin
  ...
  __dataHandler     := CDataHandler.create;
...
procedure  TfmMain.AddMsg(bSucc:boolean; sMsg:string; bShow:boolean);
begin
  if bSucc then
    m_log.log(INFO, sMsg)
  else
...
begin
  ...
  InitializeCriticalSection(m_csClientCtx);
...
begin
  ...
  DeleteCriticalSection(m_csClientCtx);
...
//    end;
    ...
    // ... message log
    AddMsg(True, 'Client Connected!' + 'Port=' + IntToStr(Port)
                      + ' '   + '(PeerIP=' + PeerIP
...
begin
  AddMsg(True, 'Client Disconn!', true);
end;
...
begin
    ...
    // ... message log
    //AddMsg(True, Format('Client Disconnected(Key:%s)(%s)', [ClientCtx(AContext.Data).sKey, peerIP]));

    AddMsg(True, Format('Client Disconnected(%s)', [peerIP]), true);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            //EnterCriticalSection(m_csClientCtx);
            IdTCPServer.Contexts.LockList;
            clCtx         := ClientCtx.Create;
            clCtx.sKey    := key;
            AContext.Data := clCtx;
            IdTCPServer.Contexts.UnlockList;
            //LeaveCriticalSection(m_csClientCtx);

            AddMsg(true, format('[Recv Logon][ctx key:%s]', [key]), true);
          end;
        end;

        __QueueEx[Q_RECV].Add(key, code, msg);

        // ... message log
        if code<>CODE_MARKET_DATA then
          AddMsg(True,'[RECV]'+ msg, false);
      end;
...
begin
    ...
    // ... message log
    AddMsg(True, 'Status:'+AStatusText, true);
end;
...
begin
    ...
    AddMsg(True, 'Client Count:'+inttostr(nclients), true);
end;
...
begin
  ...
  begin
    ...
    dataLen := length(pItem.data);
    SetLength(B, dataLen);
    CopyMemory(Addr(B[0]), Addr(pItem.data[1]), dataLen);

    fmMain.AddMsg(True, format('[Get SendQ](%s)', [pItem.data]), true);
    if fmMain.Ctx_Get(pItem.sKey, ctx) then
    begin
      try
        ctx.Connection.IOHandler.Write(pItem.data);
        fmMain.AddMsg(True, format('[SEND-%s](%s)', [pItem.sKey, pItem.data]), true);
      except
        fmMain.AddMsg(False, format('[send-except](%s)', [pItem.sKey]), true);
      end;
    end;
