...
uses
  ...
  ,CommonUtils, CommonVal, uLocalCommon;
...
begin
  ...
  begin                             
    ...
    fmMain.gdTerminal.Cells[GDTERMINAL_STATUS, m_UpdateIndex + 1]  := fmMain.MT4Info[m_UpdateIndex].StateStr;
  end
  else
  begin
    for i1 := 1 to fmMain.gdTerminal.RowCount - 1 do
    begin
      fmMain.MT4Info[i1 - 1].State    := m_nUpdateState;
      fmMain.MT4Info[i1 - 1].StateStr := StateStrings[m_nUpdateState];
      fmMain.gdTerminal.Cells[GDTERMINAL_STATUS, i1]         := fmMain.MT4Info[i1 - 1].StateStr;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      fmMain.AddMsg('Error in downloading file.Please retry again');  //fmMain.ML.GetTranslatedText('ERR_DOWNLOAD_FILES'), true, true);
...
begin
  ...
  begin
    ...
    begin
      fmMain.AddMsg('Start download/compare MT4 files');  //fmMain.ML.GetTranslatedText('MSG_START_DOWNLOAD'), false, false);
      Synchronize(ShowProgressBar);
      DownloadFiles();
      Synchronize(HideProgressBar);
      fmMain.AddMsg('Complete download/compare MT4 files', false, false); //fmMain.ML.GetTranslatedText('MSG_COMPLETE_DOWNLOAD'), false, false);
  ...
  try
    ...
    fmMain.lblMsgWait.Visible := false;
  except
  ...
  fmMain.AddMsg('Complete download/compare MT4 files');//fmMain.ML.GetTranslatedText('MSG_COMPLETE_DOWNLOAD'), false, false);
  fmMain.Update_MainApp();
