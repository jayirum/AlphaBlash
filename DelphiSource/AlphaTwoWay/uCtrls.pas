...
begin
  ...
  __ctrls[i].gdPos          := @fmMain.gdPos_A;
...
function __gdDecimalCnt(iSymbol, iSide:integer):integer;
begin
  Result := StrToInt(__ctrls[iSymbol].gdPos.cells[POS_DECIMAL, iSide]);
end;


function __gdPipSize(iSymbol, iSide:integer):double;
begin
  Result := StrToFloat(__ctrls[iSymbol].gdPos.cells[POS_PIPSIZE, iSide]);
end;
...
function __gdIsSending(iSymbol, iSide:integer):boolean;
begin
  Result := (__ctrls[iSymbol].gdPos.Cells[POS_SENDING, iSide] = 'Y');
end;
...
function  __gdPLStatus(iSymbol, iSide:integer):string;
begin
  Result := __ctrls[iSymbol].gdPos.Cells[POS_PL_STATUS, iSide];
end;

function  __gdLots(iSymbol, iSide:integer):double;
begin
  Result := strtofloatdef(__ctrls[iSymbol].gdPos.Cells[POS_LOTS, iSide],0);
end;
...
function  __gdSpread      (iSymbol, iSide:Integer):double;
begin
  Result := strtofloatdef(__ctrls[iSymbol].gdPos.Cells[POS_SPREAD, iSide],0);
end;
