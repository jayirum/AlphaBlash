...
int OnInit()
{
  ...
  if( __CheckEAInitCondition(_sMsg)==false)
   ...
   if(__Ini_GetVal(_IniFileName, "ORDER_OPTION", "CLIENT_TP", _sClientTp)==false)
      return -8;


   PrintFormat("ClientTp(%s), Timeout for MD Fetch(%d), Timeout for Reconnect(%d).TRADETIME(%s ~ %s)", _sClientTp, _TIMEOUT_MDFETCH, _TIMEOUT_RECONN, _TRADETIME_FROM, _TRADETIME_TO);
   ...
   else if (_timerTp==TIMER_STEP_2 )
   {
      ...
      if( ret==RET_OK )
      {
         while(_parser.Size()>0 )
         {
            ...
            {
               ulong uTicket;
               if(Open_NewPosition(sOnePacket, StringLen(sOnePacket), _Out_ uTicket) ) { Send_OpenPosition(_In_ uTicket); }
...
{  
   ...
   for (int i=0; i<ArraySize(_arrMD); i++ )
   {
      ...
      set.SetVal(FDS_CLIENT_TP, _sClientTp);
