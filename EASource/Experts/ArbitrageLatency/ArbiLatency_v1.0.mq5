...
int OnInit()
{
  ...
  if( __CheckEAInitCondition(_sMsg)==false)
   ...
   if(__Ini_GetVal(_IniFileName, "ORDER_OPTION", "SLIPPAGE_PIP", s)==false)
      return -3;
   _SLIPPAGE_PIP = (int)StringToInteger(s);
...
{  
   ...
   {  
      ...
      if(sCode==CODE_RETURN_ERROR)   //9002
      {
         ...
         _sMsg = StringFormat("[Recv Error Code](%s)(%s)", sCode, sOnePack);
         showErr(_sMsg);
         return RET_ERR;
      }
...
bool Place_OpenOrder(_In_ CProtoGet& get)
{
   ...
   showMsg(StringFormat("OpenOrder OK(%s)(Ticket:%d)", sSymbol,result.order));
