...
int  _Start()
{
	...
	char zLogDir[_MAX_PATH] = { 0 };
	CUtil::GetConfig(g_zConfig, "DIR", "LOG", zLogDir);
	g_log.OpenLog(zLogDir, EXENAME);

	//LOGGING(INFO, TRUE, "-----------------------------------------------------");
	//LOGGING(INFO, TRUE, "-----------------------------------------------------");
	//LOGGING(INFO, TRUE, "-----------------------------------------------------");
	//LOGGING(INFO, TRUE, "(DIR:%s)(cnfg:%s)", szDir, g_zConfig);
	//LOGGING(INFO, TRUE, "Version[%s]", __DATE__);
	...
	{
		LOGGING(ERR, TRUE, "IOCP initialize failed");
	}
	else
	{
