...
BOOL CIocp::Initialize( )
{
	...
	InitializeCriticalSection(&m_csSymbolMD);
...
BOOL CIocp::CreateSaveMD_BySymbol()
{
	char buffer[128] = { 0 };
	char zIdx[32]; 
	...
	for (int i = 0; i < nCnt; i++)
	{
		...
		CSaveMD* p = new CSaveMD(vecSplit[0]);

		LOGGING(INFO, TRUE, TRUE, "Create Save MD[%d](%s)", i, vecSplit[0].c_str());

		m_mapSymbolMD[vecSplit[0]] = p;
	}
	return TRUE;
}
...
void CIocp::Finalize()
{
	...
	DeleteCriticalSection(&m_csSymbolMD);
...
void CIocp::DispatchData(COMPLETION_KEY* pCK, const char* pRecvData, const int nRecvLen)
{
	char zCode[32] = { 0 };
	CProtoUtils utils;
	if (utils.PacketCode((char*)pRecvData, zCode) == NULL)
	{
	...
	{
		char zSymbol[32] = { 0 };
		if ( !utils.GetValue((char*)pRecvData, FDS_SYMBOL, zSymbol) )
		{
			LOGGING(ERR, TRUE, TRUE, "No symbol in the packet(%.128s)", pRecvData);
			return;
		}
		string sSymbol = string(zSymbol);
		map<string, CSaveMD*>::iterator it;
		if (!FindSymbolMDMap(sSymbol, it))
		{
			LOGGING(ERR, TRUE, TRUE, "Failed to find symbolMD (%s)", sSymbol.c_str());
			return;
		}
...
BOOL CIocp::Logon_Process(SOCKET sock, const char* pLoginData, int nDataLen)
{
	...
	if (zClientSockTp[0] == 'R')
	{
		...
		set.SetVal(FDN_ARRAY_SIZE, (int)m_mapSymbolMD.size());

		int k = 0;
		map<string, CSaveMD*>::iterator it;
		LOCK_CS(m_csSymbolMD);
		for (it = m_mapSymbolMD.begin(), k = 0; it != m_mapSymbolMD.end(); ++it, k++)
		{
	...
	while (p->m_bRun)
	{
		...
		while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
		{
			...
			if (msg.message == WM_ORDER_SEND)
			{
				...
				//EnterCriticalSection(&p->m_csSymbolMD);
				...
				//	sock = (*it).second;
				//}
				//LeaveCriticalSection(&p->m_csSymbolMD);
				//
