...
BOOL CIocp::Initialize( )
{
	...
	InitializeCriticalSection(&m_csTr);
	...
	// IOCP buffer
	m_buffering = new CProtoBuffering[m_dwThreadCount];
...
void CIocp::DeleteSocket(COMPLETION_KEY *pCompletionKey)
{
	...
	EnterCriticalSection(&m_csTr);
	map<string, CTr*>::iterator itTr;
	for (itTr = m_mapTr.begin(); itTr != m_mapTr.end(); itTr++)
		(*itTr).second->RegUnreg(pCompletionKey->sock, __ALPHA::CODE_UNREG_ROUTER);
	LeaveCriticalSection(&m_csTr);
...
void CIocp::DispatchData(COMPLETION_KEY* pCK, const char* pRecvData, const int nRecvLen)
{
	...
	try
	{
		...
		if (sCode.compare(__ALPHA::CODE_REG_ROUTER) == 0 ||
			sCode.compare(__ALPHA::CODE_REG_ROUTER) == 0 )
		{
			RegUnregTr(sCode, pCK, pRecvData, nRecvLen);
		}
		else 
		{
			RoutingData(sCode, pCK, pRecvData, nRecvLen);
		}
...
void CIocp::RegUnregTr(string sCode, COMPLETION_KEY* pCK, const char* pRecvData, const int nRecvLen)
{
	
	CTr* pTr = NULL;
	map<string, CTr*>::iterator it;
	
	EnterCriticalSection(&m_csTr);
	for (it = m_mapTr.begin(); it != m_mapTr.end(); it++)
	{
		if ((*it).first == sCode)
		{
			pTr = (*it).second;
	...
	if (pTr->RegUnreg(pCK->sock, sCode.c_str()) == FALSE)
	{
		g_log.logW(ERR, TEXT("RegUnreg Error:%s"), pTr->GetMsg());
	}
...
void CIocp::RoutingData(string sCode, COMPLETION_KEY* pCK, const char* pRecvData, const int nRecvLen)
{
	map<string, CTr*>::iterator it;
	CTr* pTr = NULL;
	EnterCriticalSection(&m_csTr);
	...
	pTr->SendData((char*)pRecvData, nRecvLen);
}
