...
wchar_t		g_wzMyName[128];

BOOL WINAPI ControlHandler(DWORD dwCtrlType);


wchar_t APP_VERSION[] = L"202006_01";
wchar_t EXE_NAME[128] = { 0 };
...
int main(int argc, LPWSTR* argv)
{
	...
	//	GET LOG DIR
	CUtil::GetMyModuleAndDir(szDir, EXE_NAME, g_wzConfig);
	...
	g_log.OpenLogEx (zSvrName, szDir, EXE_NAME, zNotiIP, _ttoi(zNotiPort), EXE_NAME);
	
	g_log.logW(LOGTP_SUCC, TEXT("-----------------------------------------------------"));
	g_log.logW(LOGTP_SUCC, TEXT("Version[%s] %s"), TEXT(__DATE__), APP_VERSION);
	g_log.logW(LOGTP_SUCC, TEXT("-----------------------------------------------------"));
