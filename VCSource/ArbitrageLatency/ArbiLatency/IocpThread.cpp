...
CIocp::CIocp()
{
	...
	m_hThread_Listen = m_hParsing = m_hThread_OrderBest = m_hThread_OrderWorst = NULL;	// m_hNoMoreOpenThread = m_hWorkerThread = NULL;
	m_unThread_Listen = m_unParsing = m_unThread_OrderBest = m_unThread_OrderWorst = 0;	// m_unNoMoreOpenThread = m_unWorkerThread = 0;
...
BOOL CIocp::LoadSymbols()
{
	...
	for (int i = 0; i < nCnt; i++)
	{
		...
		if ( !p->Initialize(vec.at(IDX_SYMBOL), 
			vec.at(IDX_POINT_SIZE), 
			vec.at(IDX_NETPROFIT_POINT), 
			vec.at(IDX_TRACE_CNT),
			vec.at(IDX_VOLUME),
			vec.at(IDX_SLIPPAGE_PTS),
			m_unThread_OrderBest,
			m_unThread_OrderWorst
			)
			)
		{
			//TODO LOGGING
...
BOOL CIocp::Logon_Process(SOCKET sock, const char* pLoginData, int nDataLen)
{
	...
	if (zClientSockTp[0] == 'R')
	{
		...
		LOGGING(INFO, TRUE, "[%s] Add to Send To Client socket", zBrokerKey);
	}
	return TRUE;
}
	...
	while (p->m_bRun)
	{
		...
		while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
		{
			...
			if (msg.message == WM_ORDER_SEND)
			{
				...
				LOGGING(INFO,TRUE, "[SendOrder](%s)(%s)", pOrd->brokerKey, pOrd->zSendBuf);
