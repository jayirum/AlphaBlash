...
void CArbiLatency::MarketDataProc(const string* pData)
{
    ...
    char zBrokerKey[128] = { 0 },  zClientTp[32] = { 0 };// , zSymbol[128] = { 0 };
    char zMDTime[32] = { 0 };
    double dBid, dAsk, dSpread;

    get.GetVal(FDS_KEY, zBrokerKey);
    get.GetVal(FDS_CLIENT_TP, zClientTp);
    ...
    Update_CurrMD(zBrokerKey, zClientTp, dBid, dAsk, dSpread, zMDTime);

    if (m_mdFaster.Is_Empty() || m_mdSlower.Is_Empty())
    {
        return;
    }
...
BOOL CArbiLatency::Check_Triggering()
{
    double dGap = 0;
    //char cBuySell;
    BOOL bTriggerdBidF = FALSE, bTriggerdAskF = FALSE;
    BOOL bTriggerdBidS = FALSE, bTriggerdAskS = FALSE;

    if (fabs(m_mdFaster.GapBid())/m_symbolInfo.dPtsSize > m_symbolInfo.nTargetPtsOpening)
    {
        bTriggerdBidF = TRUE;
    }
    if (fabs(m_mdFaster.GapAsk()) / m_symbolInfo.dPtsSize > m_symbolInfo.nTargetPtsOpening)
    {
        bTriggerdAskF = TRUE;
    }
    if (fabs(m_mdSlower.GapBid()) / m_symbolInfo.dPtsSize > m_symbolInfo.nTargetPtsOpening)
    {
        bTriggerdBidS = TRUE;
    }
    if (fabs(m_mdSlower.GapAsk()) / m_symbolInfo.dPtsSize > m_symbolInfo.nTargetPtsOpening)
    {
        bTriggerdAskS = TRUE;
    }


    if ( !bTriggerdBidF && !bTriggerdAskF && !bTriggerdBidS && !bTriggerdAskS )
        return FALSE;
    ...
    g_csv.Data_Add(m_triggered.zLocalTime, false);
    g_csv.Data_Add(m_symbolInfo.sSymbol, false);
    g_csv.Data_Add(m_mdFaster.sBroker, false);
    g_csv.Data_Add(m_mdSlower.sBroker, false);


    g_csv.Data_Add(m_mdFaster.sMDTime_curr, false);
    g_csv.Data_Add(m_mdFaster.dBid_prev, false);
    g_csv.Data_Add(m_mdFaster.dBid_curr, false);
    g_csv.Data_Add(m_mdFaster.GapBid() / m_symbolInfo.dPtsSize, false);
    if(!bTriggerdBidF)   g_csv.Data_Add(DIRECTION_FLAT, false);
    else {
        string sDirection =(m_mdFaster.GapBid() > 0) ? DIRECTION_UP : DIRECTION_DN;
        g_csv.Data_Add(sDirection, false);
    }
    g_csv.Data_Add(m_mdFaster.dAsk_prev, false);
    g_csv.Data_Add(m_mdFaster.dAsk_curr, false);
    g_csv.Data_Add(m_mdFaster.GapAsk() / m_symbolInfo.dPtsSize, false);
    if (!bTriggerdAskF)  g_csv.Data_Add(DIRECTION_FLAT, false);
    else {
        string sDirection = (m_mdFaster.GapAsk() > 0) ? DIRECTION_UP : DIRECTION_DN;
        g_csv.Data_Add(sDirection, false);
    }
    g_csv.Data_Add(m_mdFaster.dSpread_curr / m_symbolInfo.dPtsSize,false);

    g_csv.Data_Add(m_mdSlower.sMDTime_curr, false);
    g_csv.Data_Add(m_mdSlower.dBid_prev, false);
    g_csv.Data_Add(m_mdSlower.dBid_curr, false);
    g_csv.Data_Add(m_mdSlower.GapBid() / m_symbolInfo.dPtsSize, false);
    if (!bTriggerdBidS)  g_csv.Data_Add(DIRECTION_FLAT, false);
    else {
        string sDirection = (m_mdSlower.GapBid() > 0) ? DIRECTION_UP : DIRECTION_DN;
        g_csv.Data_Add(sDirection, false);
    }
    g_csv.Data_Add(m_mdSlower.dAsk_prev, false);
    g_csv.Data_Add(m_mdSlower.dAsk_curr, false);
    g_csv.Data_Add(m_mdSlower.GapAsk() / m_symbolInfo.dPtsSize, false);
    if (!bTriggerdAskS)  g_csv.Data_Add(DIRECTION_FLAT, false);
    else {
        string sDirection = (m_mdSlower.GapAsk() > 0) ? DIRECTION_UP : DIRECTION_DN;
        g_csv.Data_Add(sDirection, false);
    }
    g_csv.Data_Add(m_mdSlower.dSpread_curr / m_symbolInfo.dPtsSize, false);

    g_csv.Data_Add(m_symbolInfo.nTargetPtsOpening, false);
    g_csv.Data_Add("N", true);
    ...
    //m_triggered.sBrokerFaster = m_mdFaster.sBroker;
    //m_triggered.sBrokerSlower = m_mdSlower.sBroker;

    //if (bTriggerdF && bTriggerdS)
    //    m_triggered.sBrokerGap = DEF_BOTH;
    //else if (bTriggerdF)
    //    m_triggered.sBrokerGap = DEF_FASTER;
    //else if (bTriggerdS)
    //    m_triggered.sBrokerGap = DEF_SLOWER;

    //m_triggered.MDTimeF = m_mdFaster.sMDTime_curr;
    //m_triggered.PrevF = m_mdFaster.d
...
VOID CArbiLatency::Update_CurrMD(const char* pzBroker, LPCSTR pzClientTp, const double dBid, const double dAsk, const double dSpread, const char* pzMT4Time)
{
    CCurrMD* p;
    if (Is_Faster(pzClientTp))
        p = &m_mdFaster;
    else
        p = &m_mdSlower;
    ...
    {
        p->sBroker = pzBroker;
        p->sClientTp = pzClientTp;

        p->sMDTime_prev = pzMT4Time;
        p->dBid_prev = dBid;
        p->dAsk_prev = dAsk;
        p->dSpread_prev = dSpread;

        p->sMDTime_curr = pzMT4Time;
        p->dBid_curr = dBid;
        p->dAsk_curr = dAsk;
        p->dSpread_curr = dSpread;
    }
    else
    {
        p->Swap();
