...
CIocp::CIocp()
{
	...
	m_hThread_Listen = m_hParsing = m_hThread_OrderBest = m_hThread_OrderWorst = m_hThread_Config = NULL;	// m_hNoMoreOpenThread = m_hWorkerThread = NULL;
	m_unThread_Listen = m_unParsing = m_unThread_OrderBest = m_unThread_OrderWorst = m_unThread_Config = 0;	// m_unNoMoreOpenThread = m_unWorkerThread = 0;
...
BOOL CIocp::Initialize( )
{
	...
	m_hThread_OrderBest = (HANDLE)_beginthreadex(NULL, 0, &Thread_OrderSend, this, 0, &m_unThread_OrderBest);
	m_hThread_OrderWorst= (HANDLE)_beginthreadex(NULL, 0, &Thread_OrderSend, this, 0, &m_unThread_OrderWorst);
...
BOOL CIocp::CreateSymbolThread()
{
	//char buffer[128] = { 0 };
	//CUtil::GetConfig(g_zConfig, TEXT("SYMBOLS"), TEXT("COUNT"), buffer);
	//if (buffer[0] == NULL)
	//{
	//	LOGGING(ERR, TRUE, "Failed to get symbols count from config file");
	//	return FALSE;
	//}
	//
	//char zTimeFrom[32] = {}, zTimeTo[32] = {};
	//CUtil::GetConfig(g_zConfig, TEXT("MARKET_TIME"), TEXT("TRADETIME_FROM"), zTimeFrom);
	//if (buffer[0] == NULL)
	//{
	//	LOGGING(ERR, TRUE, "Failed to get TRADETIME_FROM from config file");
	//	return FALSE;
	//}
	//CUtil::GetConfig(g_zConfig, TEXT("MARKET_TIME"), TEXT("TRADETIME_TO"), zTimeTo);
	//if (buffer[0] == NULL)
	//{
	//	LOGGING(ERR, TRUE, "Failed to get TRADETIME_TO from config file");
	//	return FALSE;
	//}

	vector<string> vecSymbols;
	int nCnt = g_config.getSymbols(&vecSymbols);

	for (int i = 0; i < nCnt; i++)
	{
		TSymbolInfo info;
		if (g_config.getSymbolInfo(vecSymbols.at(i), &info) == FALSE)
			return FALSE;

		CArbiLatency* p = new CArbiLatency();
		if ( !p->Initialize(&info, m_unThread_OrderBest,m_unThread_OrderWorst))
		{
			//TODO LOGGING
			return FALSE;
		}
		
		m_mapSymbol[info.sSymbol] = p;

		LOGGING(INFO, TRUE, "Load Symbol[%s](TargetOpen:%d)(TargetClose:%d)",
			info.sSymbol.c_str(),
			info.nTargetPtsOpening,
			info.nTargetPtsClosing
		);
	}
	return TRUE;
}
			...
			do
			{
				...
				if (nRet > 0)
				{
					...
					if (strcmp(zCode, __ALPHA::CODE_LOGON) == 0)
					{
						...
						p->pCK = pCK;
						p->packet = string(zRecvBuff);
						PostThreadMessage(pThis->m_unThread_Login, WM_LOGIN, (WPARAM)0, (LPARAM)p);
						continue;
					}
...
void CIocp::AddList_ClientRecvSock(char* pzBrokerKey, SOCKET sock)
{
	...
	map<string, SOCKET>::iterator it = m_mapToSend.find(sBroker);
	if (it != m_mapToSend.end())
	{
		LOGGING(ERR, TRUE, "Map to Send has the same broker already(%s)(%d)", sBroker.c_str(), sock);
		m_mapToSend.erase(sBroker);
	}

	m_mapToSend[sBroker] = sock;
	LeaveCriticalSection(&m_csToSend);

	LOGGING(INFO, TRUE, "[%s] Add to Send To Client socket(%d)", sBroker.c_str(), sock);
}
...
unsigned WINAPI CIocp::Thread_Login(LPVOID lp)
{
	...
	while (pThis->m_bRun)
	{
		...
		while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
		{
			if (msg.message != WM_LOGIN)
				continue;

			TPacket* p = (TPacket*)msg.lParam;
			LOGGING(INFO, FALSE, "[LOGIN-2]%d", p->pCK->sock);
			pThis->Logon_Process(p->pCK->sock, p->packet.c_str(), p->packet.size());
			delete p;			
		}
	}
...
BOOL CIocp::Logon_Process(SOCKET sock, const char* pLoginData, int nDataLen)
{
	...
	{
		LOGGING(ERR, TRUE, "(%s)(%s)", get.GetMsg(), pLoginData);
		return FALSE;
	}
	int res = 0;
	...
	if (res < 2)
	{
		LOGGING(ERR,TRUE, "Failed to get BrokerKey and Socket Type from Logon Packet");
		return FALSE;
	}
...
unsigned WINAPI CIocp::Thread_OrderSend(LPVOID lp)
{
	...
	while (p->m_bRun)
	{
		...
		{
			BOOL bDelete = TRUE;
			if (msg.message == WM_ORDER_OPEN || msg.message == WM_ORDER_CLOSE)
			{
				string sOrderTp = (msg.message == WM_ORDER_OPEN) ? "OPEN" : "CLOSE";
				...
				LOGGING(INFO,TRUE, "[SOCK:%d][SendOrder-%s](%s)(%d)(%s)", sock, sOrderTp.c_str(), pOrd->brokerKey, sock, pOrd->zSendBuf);
...
unsigned WINAPI CIocp::Thread_Config(LPVOID lp)
{
	...
	while (p->m_bRun)
	{
		...
		else if (ret == CNFG_SUCC)
		{
			...
			p->lockSymbol();
			for(IT_MAP_SYMBOL it = p->m_mapSymbol.begin(); it!=p->m_mapSymbol.end(); ++it)
			{
				(*it).second->Update_Config();
			}
			p->unlockSymbol();
		}
...
//{
//	char zTemp[1024] = { 0, };
//	if (!CUtil::GetConfig(g_zConfig, TEXT("BROKER"), TEXT("COUNT"), zTemp))
//		return FALSE;
//	m_nBrokersCnt = atoi(zTemp);
//	return TRUE;
//}
//
