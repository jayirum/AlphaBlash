...
char* CTimeUtils::DateTime_yyyymmdd_hh_mm_ss_mmm(_Out_ char* pzDate)
{
	SYSTEMTIME st; GetLocalTime(&st);
	sprintf(pzDate, "%04d%02d%02d_%02d:%02d:%02d:%03d", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
	return pzDate;
}

char* CTimeUtils::Today_yyyymmdd(_Out_ char* pzDate)
{
	SYSTEMTIME st; GetLocalTime(&st);
	sprintf(pzDate, "%04d%02d%02d",st.wYear, st.wMonth, st.wDay);
	return pzDate;
}

char* CTimeUtils::Time_hhmmssmmm(_Out_ char* pzTime)
{
	SYSTEMTIME st; GetLocalTime(&st);
	sprintf(pzTime, "%02d%02d%02d%03d", st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
	return pzTime;
}

char* CTimeUtils::Time_hh_mm_ss_mmm(_Out_ char* pzTime)
{
	SYSTEMTIME st; GetLocalTime(&st);
	sprintf(pzTime, "%02d:%02d:%02d:%03d", st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
	return pzTime;
...
char* CTimeUtils::DateTime_yyyymmdd_hhmmssmmm(_Out_ char* pzTime)
{
	SYSTEMTIME st; GetLocalTime(&st);
	sprintf(pzTime, "%04d%02d%02d_%02d%02d%02d%03d", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
	return pzTime;
}
