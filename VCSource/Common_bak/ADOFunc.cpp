...
CADOFunc::CADOFunc()
{
	...
	m_pRs = NULL;
	m_pCmd = NULL;
	m_pzCmdBuffer = new TCHAR[LEN_CMDBUFF];
...
BOOL CADOFunc::Open(TCHAR *pzServerIP, TCHAR *pzUser, TCHAR *pzPassword, TCHAR* pzDBName)
{
	_ParameterPtr	pParam;
	m_bErrFlag = FALSE;
	_stprintf(m_zConnStr, TEXT("Provider=SQLOLEDB.1;Data Source=%s;User Id=%s;Password=%s;Initial Catalog=%s"), 
...
BOOL CADOFunc::QrySelect(TCHAR* pzQry)
{
...
/*
	일반 쿼리문 실행.(SP 아님)
*/
BOOL CADOFunc::ExecQuery(TCHAR* pzQry)
{
...
BOOL CADOFunc::AddParam(TCHAR *pName, enum DataTypeEnum eType, enum ParameterDirectionEnum eDirection, LONG lSize, VARIANT vtValue)
{
	_ParameterPtr	pParam;
	m_bErrFlag = FALSE;
...
BOOL CADOFunc::DelParam(TCHAR* pName)
{
...
BOOL CADOFunc::SetParamValue(TCHAR* pName, VARIANT vtValue)
{
	_bstr_t bstrName(pName);
	if(m_pCmd->Parameters->GetItem(bstrName) == NULL)
		return FALSE;
...
//VOID CADOFunc::GetValueEx2(TCHAR* pszField, VARIANT* pRet)
//{
//	VariantCopy(pRet, &m_pRs->Fields->Item[_bstr_t(pszField)]->Value);
//}

TCHAR* CADOFunc::GetStrWithLen(TCHAR* pszField, int nMaxLen, TCHAR* pzOut)
{
	_variant_t vVal;
	...
	{
		_stprintf(m_szMessage, TEXT("GetStrWithLen try catch error"));
		return NULL;
	}
	//_tcscpy(pzOut, (const TCHAR*)(_bstr_t)GetValue(pszField)); 
	_stprintf(pzOut, TEXT("%.*s"), nMaxLen, (TCHAR*)(_bstr_t)vVal);
	return pzOut;
}


TCHAR* CADOFunc::GetStr(TCHAR* pszField, TCHAR* pzOut)
{
	VARIANT vVal;
	...
	//_tcscpy(pzOut, (const TCHAR*)(_bstr_t)GetValue(pszField)); 
	_tcscpy(pzOut, (const TCHAR*)(_bstr_t)vVal);
	return pzOut;
}



int CADOFunc::GetStrEx(TCHAR* pszField, TCHAR* pzOut, int * pnLen)
{
	*pnLen = -1;
	__try {
	...
	__except (ReportException(GetExceptionCode(), TEXT("CADOFunc::GetStr"), pzOut))
	{
	}
	return *pnLen;
}
...
VARIANT CADOFunc::GetValue(TCHAR* pszField)
{
	return m_pRs->Fields->Item[_bstr_t(pszField)]->Value;
}


BOOL CADOFunc::GetValueEx(TCHAR* pszField, VARIANT* pRet)
{
	BOOL bRet = TRUE;
	__try {
		*pRet = GetValue(pszField);
	}
	__except (ReportException(GetExceptionCode(), TEXT("CADOFunc::GetValueEx"), m_szMessage))
	{
		bRet = FALSE;
	}
	return bRet;
...
long CADOFunc::ExecSP(TCHAR *pszFormat, ...)
{
	va_list argptr;

	va_start(argptr, pszFormat);
	_vstprintf(m_pzCmdBuffer, pszFormat, argptr);
	va_end(argptr);
...
long CADOFunc::ExecSP(TCHAR *pszFormat, TCHAR *pszRecordArray)
{
		...
		else
		{
			...
			for(long k=0; k < lRecCnt; k++)
			{
				for(long i=0; i<m_pRs->Fields->Count; i++)
					lstrcat(pszRecordArray, (const TCHAR*)(_bstr_t)GetValue(i));
...
/*
BOOL CADOFunc::Execute(TCHAR * szCmdText, VARIANT vtParam)
{
	if(m_pRs != NULL && m_pRs->State != adStateClosed){
		m_pRs->Close();
		m_pRs = NULL;
...
void CADOFunc::DumpError(_com_error &e,TCHAR *szBuffer)
{
	m_bErrFlag = TRUE;
	...
	TCHAR szConnErr[2048];
	szConnErr[0] = 0x00;
    if( (m_pConn->Errors->Count) > 0)
    {
        pErr = m_pConn->Errors->GetItem((long)(m_pConn->Errors->Count - 1));
		_stprintf(szConnErr, TEXT("%s"), (const TCHAR*)pErr->Description);
    }

	_stprintf(m_szMessage,TEXT("CONNECTION 객체 에러->%s<- \n DESCRIPTION->%s<- \n DSN/SP명->%s<- \n%s 에러코드->%ld<-"),
			szConnErr, (const TCHAR*)bstrDescription, szBuffer, (const TCHAR*)bstrSource, (long)e.Error());
}

TCHAR* CADOFunc::GetError()
{
	return m_szMessage;
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//<DBPool>
CDBPoolAdo::CDBPoolAdo( TCHAR *pzServerIP, TCHAR *zUserID, TCHAR *zPassword, TCHAR* zDBName)
{
	InitializeCriticalSectionAndSpinCount(&m_cs, 2000);
	_tcscpy(m_zServer, pzServerIP);
	_tcscpy(m_zID, zUserID);
	_tcscpy(m_zPwd, zPassword);
	_tcscpy(m_zDBName, zDBName);
	CoInitialize(NULL);
}
...
BOOL CDBPoolAdo::Init(int nInitCnt)
{
	if (nInitCnt <= 0)
	{
		_stprintf(m_zMsg, TEXT("DB POOL Count Err:%d"), nInitCnt);
		return FALSE;
	}
	for (int i = 0; i < nInitCnt; i++)
	{
		...
		{
			_stprintf(m_zMsg, TEXT( "DB init err(SERVER:%s)(DB:%s)(ID:%s)(Pwd:%s)(%s)"),
...
CADOFunc* CDBPoolAdo::GetAvailableDB()
{
	if (m_setDB.size() == 0)
	{
		if (!AddNew())
		{
			_stprintf(m_zMsg, TEXT("There is no session in the pool"));
			return NULL;
		}
	}
