...
VOID CDlgLogin::ReadConfig()
{
	char temp[128];
	CUtil::GetConfig(g_zConfig, "EBEST", "IP", m_zEbestIP);
	CUtil::GetConfig(g_zConfig, "EBEST", "PORT", temp);
	m_nEbestPort = atoi(temp);

	CUtil::GetConfig(g_zConfig, "EBEST", "ID", m_zApiID);
	CUtil::GetConfig(g_zConfig, "EBEST", "CONN_PWD", m_zConnPwd);
	CUtil::GetConfig(g_zConfig, "EBEST", "CERT_PWD", m_zCertPwd);

	g_log.log(INFO, "(%s)(%d)(%s)(%s)(%s)", m_zEbestIP, m_nEbestPort, m_zApiID, m_zConnPwd, m_zCertPwd);
}
...
{	
	...
	BOOL bResult = g_iXingAPI.Connect(
		...
		-1,						// 지정한 시간이상(1/1000 초 단위)으로 시간이 걸리게 될 경우 연결실패로 간주함
	...
	if (bResult == FALSE)
	{
		...
		CString strMsg = g_iXingAPI.GetErrorMessage(nErrorCode) + '[' + m_zEbestIP + ']';
		MessageBox(strMsg, "서버접속실패", MB_ICONSTOP);

		g_log.log(NOTIFY, "Connect Ebest Error(%d)(%s)", nErrorCode, (LPCSTR)strMsg);
...
BOOL CDlgLogin::Login()
{
	...
	if (bResult == FALSE)
	{
		...
		MessageBox(strMsg, "XingAPI 로그인 실패", MB_ICONSTOP);

		g_log.log(NOTIFY, "Login Ebest Error(%d)(%s)", nErrorCode, (LPCSTR)strMsg);

		return FALSE;
	}


	g_log.log(INFO, "Login Send OK(%s)(%s)(%s)", m_zApiID, m_zConnPwd, m_zCertPwd);
	return TRUE;
}
