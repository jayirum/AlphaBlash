...
BOOL CXingAPI_SampleApp::InitInstance()
{
	...
	CHAR	zExe[512] = { 0 };
	CUtil::GetMyModuleAndDir(szDir, zExe, g_zConfig);
	CUtil::GetConfig(g_zConfig, "DIR", "LOG", szDir);
	
	char zSvrName[32], zNotiIP[32], zNotiPort[32];
	CUtil::GetConfig(g_zConfig, "NOTIFICATION", "MYSERVER_NAME", zSvrName);
	CUtil::GetConfig(g_zConfig, "NOTIFICATION", "NOTIFICATION_SERVER_IP", zNotiIP);
	CUtil::GetConfig(g_zConfig, "NOTIFICATION", "NOTIFICATION_SERVER_PORT", zNotiPort);

	g_log.OpenLogEx(zSvrName, szDir, zExe, zNotiIP, atoi(zNotiPort), zExe);
